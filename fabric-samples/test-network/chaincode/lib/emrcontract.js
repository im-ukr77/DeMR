...
class EMRContract extends Contract {
  ...
  async getEMR(ctx, emrID, email) {
    ...
    const userKey = ctx.stub.createCompositeKey("User", [email]);
    ...
    if (emr.checkUserInPermittedList(email)) {
      // Return the EMR content along with the recordOwner (patient's email)
      return JSON.stringify({
        content: emr.getContent(),
        recordOwner: emr.getRecordOwner(),
      });
    } else {
      throw new Error(
